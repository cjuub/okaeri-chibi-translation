...
mkdir -p ./nsbmd_image_original

for i in ./tmp/nsbmd_original/*.nsbmd ; do
	if [ -d "$i" ]; then
		continue;
	fi

	name=$(basename "${i:0:-6}");
	mkdir -p ./nsbmd_image_original/"$name"
	eval "./image_tool -e3 ./tmp/nsbmd_original/"$(basename "$name")".nsbmd" "./nsbmd_image_original/"$(basename "$name");
done