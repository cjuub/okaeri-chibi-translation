...
for i in ./ncer_image_translated/* ; do
    ...
    eval "./image_tool -i ./nxxx_original/"$(basename "$name")".NCGR" "./nxxx_original/"$(basename "$name")".NCER" "./tmp/"$(basename "$name") "./tmp/ncgr_translated/"$(basename "$name")".NCGR";
done

for i in ./nscr_image_translated/* ; do
    name=$(basename "$i");
    eval "./png_merge ./nscr_image_original/"$name "./nscr_image_translated/"$name "./tmp/"$name;
    eval "./image_tool -i2 ./nxxx_original/"${name:0:-4}".NCGR" "./nxxx_original/"${name:0:-4}".NSCR" "./tmp/"$(basename "$name") "./tmp/ncgr_translated/"${name:0:-4}".NCGR";
done